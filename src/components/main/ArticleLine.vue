<script setup lang="ts">
import { Article } from "@/articles.ts";
import { useRouter } from "vue-router";
import Tag from "@/components/main/Tag.vue";
const router = useRouter();
const props = defineProps<{
  article: Article;
  new: boolean;
}>();
</script>

<template>
  <div
    class="relative flex w-4/5 flex-col flex-wrap items-start gap-3 rounded-lg border-solid py-4 pr-8 pl-4 shadow transition bg-slate-1 border-1.5 border-slate-1 duration-400 group hover:border-slate-3 hover:cursor-pointer"
    @click="router.push(article.slug)"
  >
    <div
      class="absolute top-4 right-4 transition duration-500 group-hover:rotate-45 group-hover:scale-110"
    >
      <img src="@/assets/arrow.svg" alt="arrow" class="h-6 w-6" />
    </div>

    <span class="text-lg font-800 pr-4">{{ props.article.title }}</span>
    <div class="-mt-2 flex flex-wrap gap-2" v-if="props.article.tags">
      <Tag :text="tag" v-for="tag in props.article.tags"> </Tag>
    </div>
    <div class="flex justify-between items-end gap-4 w-full mt-2">
      <span class="line-clamp-2 text-gray">{{
        props.article.description
      }}</span>

      <div class="hidden sm:flex min-w-48 gap-4 justify-end text-gray text-sm">
        <div class="flex items-center gap-1">
          <img src="@/assets/calendar.svg" alt="calendar" class="h-4 w-4" />
          <span>{{ props.article.date }}</span>
        </div>
        <div class="flex items-center gap-1">
          <img src="@/assets/clock.svg" alt="clock" class="h-4 w-4" />
          <span>{{ props.article.time }}</span>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped></style>
